...
userApp.post("/user-login",expressAsyncHandler(async(request,response)=>{
    ...
    //verify username
    let userOfDB=await userCollectionObj.findOne({name:userCredObj.name})
    ...
    //if username is valid
      ...
      //if passsword matches
        ...
        //create jwt token
       let jwtToken=jwt.sign({name:userOfDB.name},process.env.SECRET_KEY,{expiresIn:"2m"})  
...
userApp.post("/user-signup",multerObj.single('photo'),expressAsyncHandler(async(request,response)=>{
   ...
   let userOfDB=await userCollectionObj.findOne({name:newUser.name})
