...
function CurriesRoties() {
  ...
  let getCurriesRotisrecipe=()=>{
    // Fetch all recipes from the backend API
    axios.get('http://localhost:3500/recipe-api/get-recipes')
      .then(response => {
        // Filter recipes with category "curries-rotis"
        if(response.status===200)
        {
        let CurriesRotisRecipes = response.data.payload.filter(recipe => recipe.category === 'curries-rotis');
   
          setRecipes(CurriesRotisRecipes);
          setFilteredRecipes(CurriesRotisRecipes);
        }
        else{
