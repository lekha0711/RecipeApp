...
function Milkshakes() {
  ...
  let getMilkshakerecipe=()=>{
    // Fetch all recipes from the backend API
    axios.get('http://localhost:3500/recipe-api/get-recipes')
      .then(response => {
        // Filter recipes with category "milkshakes"
        if(response.status===200)
        {
        let MilkshakeRecipes = response.data.payload.filter(recipe => recipe.category === 'milkshakes');
   
          setRecipes(MilkshakeRecipes);
          setFilteredRecipes(MilkshakeRecipes);
        }
        else{
