...
function RiceItems() {
  ...
  let getRiceItemrecipe=()=>{
    // Fetch all recipes from the backend API
    axios.get('http://localhost:3500/recipe-api/get-recipes')
      .then(response => {
        // Filter recipes with category "rice-items"
        if(response.status===200)
        {
        let RiceItemRecipes = response.data.payload.filter(recipe => recipe.category === 'rice-items');
   
          setRecipes(RiceItemRecipes);
          setFilteredRecipes(RiceItemRecipes);
        }
        else{
